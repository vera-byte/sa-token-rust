...
### 3. 创建并使用拦截器 | Create and Use Interceptor

 tonic-build 编译后会生成 `auth_service_server::AuthServiceServer`，使用 `with_interceptor()` 应用拦截器：

```rust
use sa_token_plugin_tonic::{SaTokenState, GrpcServerInterceptor, MemoryStorage};
use std::sync::Arc;
...
// 中文: 使用 with_interceptor 将拦截器应用到服务
// English: Apply interceptor to service using with_interceptor
// 注意: AuthServiceServer 由 tonic-build 从 proto 文件自动生成
// Note: AuthServiceServer is auto-generated by tonic-build from proto file
let service = auth::auth_service_server::AuthServiceServer::with_interceptor(
    MyAuthServiceImpl::new(state.clone()),
    interceptor,
);
...
### 4. 在服务实现中验证 Token | Validate Token in Service Implementation

```rust
use sa_token_plugin_tonic::{validate_token_from_request, SaTokenState};
use tonic::{Request, Response, Status};

pub struct MyAuthServiceImpl {
    state: SaTokenState,
}

#[tonic::async_trait]
impl auth::auth_service_server::AuthService for MyAuthServiceImpl {
    async fn get_user_info(
        &self,
        request: Request<auth::UserInfoRequest>,
    ) -> Result<Response<auth::UserInfoResponse>, Status> {
        // 中文: 验证 Token 并获取登录 ID（异步）
        // English: Validate token and get login ID (async)
...
|--------|------------|-----------|-------------|--------|-------------|
| 工作方式 | Working Mode | HTTP Middleware | 直接透传 | Direct Pass-through |
| 认证机制 | Auth Mechanism | Middleware | Interceptor | Interceptor |
| Token 来源 | Token Source | HTTP Headers | gRPC Metadata | gRPC Metadata |
